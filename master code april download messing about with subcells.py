...
class Environment: 
    ...
    def create_subcells(self, Ncell_i, t):
        ...
        for c in self.space: #loop through all the large cells   
           ...
           # print(f'particle number before {c.particleno[t-1]}, particle no now {c.particleno[t]}')
            if t==0 or c.particleno[t-1] != c.particleno[t]: #assuming t here is a parameter we can feed in- and I think it is, as we're using a counter sort of method
                ...
                for s in c.subcellspace: #loop through all subcells in the cell
                    ...
                    countatomsappend.append(np.size(s.stored_atoms))
                #print(f'intermediate atom number = {np.sum(countatoms)}')
                    
            else: #ie if we're not making fresh subcells
               # print('doing else loop')
                for s in c.subcellspace: #loop through all subcells in the cell
                    s.stored_atoms.clear()
                    for n in c.stored_atoms: #loop through atoms in the specific cell we're considering
                        #s.stored_atoms = [n for n in c.stored_atoms if s.centre_x - self.L/(2*Ncell_i*c.subcellno) < n.x <= s.centre_x + self.L/(2*Ncell_i*c.subcellno)  and s.centre_y - self.L/(2*Ncell_i*c.subcellno) < n.y <= s.centre_y + self.L/(2*Ncell_i*c.subcellno) and s.centre_z - self.L/(2*Ncell_i*c.subcellno) < n.z <= s.centre_z + self.L/(2*Ncell_i*c.subcellno)]
                        if s.centre_x - self.L/(2*Ncell_i*c.subcellno) < n.x <= s.centre_x + self.L/(2*Ncell_i*c.subcellno)  and s.centre_y - self.L/(2*Ncell_i*c.subcellno) < n.y <= s.centre_y + self.L/(2*Ncell_i*c.subcellno) and s.centre_z - self.L/(2*Ncell_i*c.subcellno) < n.z <= s.centre_z + self.L/(2*Ncell_i*c.subcellno):
                                s.stored_atoms.append(n)
                    
                    countatomsloop.append(np.size(s.stored_atoms))
                #print('we went to else statement')
