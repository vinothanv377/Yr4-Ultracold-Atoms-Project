...
class Environment: 
    ...
    def evap_cooling(self, therm_time, Nt, rate_of_evap):
        ...
        r_vals = []
        for i in self.particles:
            r_vals.append(Particle.mag_r(i))
        cut_off = max(r_vals)
        evap_times = np.linspace(0, Nt, int(Nt/(therm_time-1)), endpoint=True)
        print(f'The Evap Times are {evap_times}')
        print(f'Starting time t ={t}')
        print(f'Starting Cut-Off is {cut_off}')
        while t != Nt:
            if np.any(t == evap_times):
                print('yes - fallen on a evap timestep')
                print(t)
                for i in self.particles:
                    if Particle.mag_r(i) >= cut_off:
                        print('Evaporated')
