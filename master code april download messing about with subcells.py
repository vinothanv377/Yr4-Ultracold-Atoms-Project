...
class Environment: 
    ...
    def create_subcells(self, Ncell_i, t):
        ...
        for c in self.space: #loop through all the large cells   
            c.particleno.append(np.size(c.stored_atoms))
            #ideally would only make new cells if we needed to- would need to access infomation about previous timesteps
            #if c.particleno[t-1] != c.particleno[t]: #assuming t here is a parameter we can feed in- and I think it is, as we're using a counter sort of method
            if np.size(c.stored_atoms)>=0:
                ...
                #determining no. of subcells needed based on how many atoms in cell
                if np.size(c.stored_atoms) > 20:
                    c.subcellno = 5
                    print(np.size(c.stored_atoms))
                    print('make 6 subcells')
                elif 20 >= np.size(c.stored_atoms) > 15:
                    c.subcellno = 4
                    print(np.size(c.stored_atoms))
                    print('make 5 subcells')
                elif 15 >= np.size(c.stored_atoms) > 10:
                    c.subcellno = 3
                    print(np.size(c.stored_atoms))
                    print('make 4 subcells')
                elif 10 >= np.size(c.stored_atoms) > 5:
                    c.subcellno = 2
                    print(np.size(c.stored_atoms))
                    print('make 3 subcells')
                elif 5 >= np.size(c.stored_atoms)>= 0:
                    c.subcellno = 1 #ie subcell is just the cell
                    print(np.size(c.stored_atoms))
                    print('make 1 subcells')
                ...
                for x in range(c.subcellno):
                 ...
                 #so I'm thinking it might be okay to do the append method as all of this is only happening if we make new subcells from fresh, so reappending lots of atoms is not a concern here
                for s in c.subcellspace: #loop through all subcells in the cell
                    ...
                    print(f'number of atoms in subcell={np.size(s.stored_atoms)}')
                    countatoms.append(np.size(s.stored_atoms))
                            
            else: #ie if we're not making fresh subcells
                for s in c.subcellspace: #loop through all subcells in the cell
                    for n in c.stored_atoms: #loop through atoms in the specific cell we're considering
                        s.stored_atoms = [n for n in self.particles if s.centre_x - self.L/(2*Ncell_i*c.subcellno) < n.x < s.centre_x + self.L/(2*Ncell_i*c.subcellno)  and s.centre_y - self.L/(2*Ncell_i*c.subcellno) < n.y < s.centre_y + self.L/(2*Ncell_i*c.subcellno) and s.centre_z - self.L/(2*Ncell_i*c.subcellno) < n.z < s.centre_z + self.L/(2*Ncell_i*c.subcellno)]
                #sort atoms into subcells the nonappending version
        print(f'number of atoms as counted in the subcell array: {np.sum(countatoms)}')
        print(f'number of entries in subcell count atoms array: {np.size(countatoms)}')
